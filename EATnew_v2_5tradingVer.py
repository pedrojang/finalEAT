            ...
            if not(actiontime == nownow()):
                ...
                if MTG == True:
                    ...
                    if MTGS == False:
                        MTGtime = MTGtime + 1
                if MTGG == True:
                    if (LP == True and (ls_ochi[-1] < ls_bolhigh[-1] or ls_high[-1] > ls_bolhigh[-1]*1.03)):
                        LP = False
                        MTGG = False
                        sellbit(PN)
                        sellnum = getcurrent()
                        ...
                        mail(text,title)
                        print('possition END')
                    if (SP == True and (ls_oclo[-1] > ls_bollow[-1]or ls_low[-1] < ls_bollow[-1]*0.97)):
                        SP = False
                        MTGG = False
                        buybit(PN)
                        buynum = getcurrent()
                ...
                if MTGGSM(-6):
                    ...
                    title = 'P-SELL'
                    mail(text,title)
                print(LP,'|',SP,'|',ls_close[-1])
                ENDtime = ENDtime + 1
        time.sleep(1)
        if LP == True and getcurrent() > ls_bolhigh[-1]*1.002 and getcurrent() > buynum*1.02:
            LP = False
            sellbit(PN)
            sellnum = getcurrent()
            time.sleep(1)
            ...
            mail(text,title)
            print('possition END')
        if SP == True and getcurrent() < ls_bollow[-1]*0.998 and getcurrent() < sellnum*0.98:
            SP = False
            buybit(PN)
            buynum = getcurrent()
            time.sleep(1)
